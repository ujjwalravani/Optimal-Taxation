function [end_gini_ga,bestsol,bestfitness] = GeneticAlgorithmmultinp(prob,lb,ub,Np,T,etac,etam,Pc,Pm,rev)
...
for p = 1:Np
    [f(p),~] = prob(P(p,:),rev);            % Evaluating the fitness function of the initial population
end
...
for t = 1: T
    ...
    for j = 1:Np
        [OffspringObj(j),~] = prob(offspring(j,:),rev);     % Evaluating the fitness of the offspring solution
    end
